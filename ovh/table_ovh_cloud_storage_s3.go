...
func listS3StorageContainer(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	...
	// DIAGNOSTIC: Log every invocation to track duplicates
	plugin.Logger(ctx).Warn("ovh_cloud_storage_s3.listS3StorageContainer", "DIAGNOSTIC_INVOCATION", "project_id", projectId, "region", region)
