...
- `regions` (Optional) - A list of regions to query. Supports wildcards (e.g., `GRA*` matches `GRA1`, `GRA5`, etc.). If not specified, all regions will be queried. This can significantly improve query performance when you only need data from specific regions.
...
```hcl
regions = ["GRA5", "SBG3", "BHS5"]
```

Query all regions in a location using wildcards:
```hcl
regions = ["GRA*"]  # Matches GRA1, GRA5, GRA7, etc.
```
